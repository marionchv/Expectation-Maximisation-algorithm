...
$\mathcal{L}(R((z_i)_i) ; \alpha, \theta) = \sum_{(z_i)_i} R((z_i)_i)\, \log \frac{p((z_i)_i, (x_i)_i \mid \alpha, \theta)}{R((z_i)_i)}$
...
$L_{(x_i)_i}(\alpha,\theta)$ ne dépend pas de $R$. Ainsi, $\mathcal{L}$ est maximum lorsque $KL$ est minimal, cad lorsque  
...
→ **$p(z_i \mid x_i,\alpha, \theta)$ ne dépend que des $\tau_{ij}$ donc l'étape E consistera à calculer les $\tau_{ij}$.**
...
On a :  
$\mathcal{L}(R((z_i)_i) ; \alpha, \theta) = \mathbb{E}_{(z_i)_i}\big[\log(p((z_i)_i, (x_i)_i \mid \alpha, \theta))\big] + \text{cte}$

On va donc chercher à maximiser :  
$\mathbb{E}_{(z_i)_i}\big[\log(p((z_i)_i, (x_i)_i \mid \alpha, \theta))\big] = \sum_{i=1}^n \sum_{j=1}^M \tau_{ij} \log(\alpha_j \mathcal{N}(x_i;\mu_j,\Sigma_j))$
